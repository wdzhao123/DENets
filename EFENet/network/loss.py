...
class hxqloss0515(nn.Module):
    def __init__(self):
        super(hxqloss0515, self).__init__()
    def forward(self, pred, gt, f):
        gt=gt.float()
        gt=gt/255.0
        loss_fn = nn.L1Loss()
        ...
        cishu=model_num*(model_num-1)/2
        similarity_sum=similarity_sum/cishu*3
